...
Purpose: vertex shader for phong_shading
Language: GLSL
Platform: win64
...
vec3 phong_shading()
{
	...
	for(int i = 0; i < count; ++i)
	{
		...
		vec3 diffuse = diff * render_datas[i].diffuse * vec3(texture(material.diffuse, uv));
		//point
		...
		else if (render_datas[i].type == 2)
		{
			...
			//vec3 reflectDir = reflect(-lightDir, norm);  
			float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
			vec3 specular = render_datas[i].specular * spec * vec3(texture(material.specular, uv));;// * texture(material.specular, TexCoords).rgb;  
...
void main()
{
	...
	vec3 color = s * phong_shading() + (1 - s) * I_fog;
